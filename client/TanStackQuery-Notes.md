...
```
This holds data fetched from the API. It's a local copy of what's on the server. We also have a `useEffect` to fetch it and manual functions (`handleSubmitForm`, `handleDeleteActivity`) to keep it in sync.
...
```
These track UI behavior — which activity is selected and whether the form is open. This data doesn't exist on the server. It's purely local.
...
| What | Code | Problem |
|------|------|---------|
| Storing fetched data | `useState<Activity[]>([])` | We manually manage the data array |
| Fetching data | `useEffect` + `axios.get` | No caching, no loading state, no error handling, runs once on mount |
| Updating after edit | `setActivities(activities.map(...))` | Manual array manipulation, no API call, easy to get out of sync |
| Updating after create | `setActivities([...activities, newActivity])` | Manual append with fake id, no API call |
| Updating after delete | `setActivities(activities.filter(...))` | Manual filter, no API call |

**After (TanStack Query):**

| What | TanStack Query | Benefit |
|------|---------------|---------|
| Fetch + store data | `useQuery({ queryKey: ['activities'], queryFn })` | Auto caching, loading/error states, refetch on focus |
...
Every component in the chain needs to declare the prop in its `Props` type, destructure it, and pass it along — even if it doesn't use the data itself (`ActivityDashboard` and `ActivityList` are just middlemen).

With TanStack Query, any component can call `useQuery({ queryKey: ['activities'] })` and get the cached data directly. No drilling. The middleman components don't need to know about activities at all.
...
|----------|--------|---------------------|
| Fetching data | `useState` + `useEffect` + `axios` | `useQuery` |
...
| Refetching | None | On window focus, reconnect, configurable |
| Prop drilling for data | Required | Eliminated |
| UI state (editMode, selectedActivity) | `useState` | Still `useState` (unchanged) |
